    ...
    generate
        for(i=0; i<ROW; i=i+1) begin : Row
            for(j=0; j<COL; j=j+1) begin : Col
                ...
                .PSUM_ADDR_BITWIDTH(PSUM_ADDR_BITWIDTH), .PSUM_DEPTH(PSUM_DEPTH)) pe_new(.clk(clk), .reset(reset), .en(MAC_en[COL*i+j]), .actv_sel(actv_sel[COL*i+j]), .actv_en(actv_en[COL*i+j]), .wgt_sel(wgt_sel[COL*i+j]), .wgt_en(wgt_en[COL*i+j]), .psum_en(psum_en[COL*i+j]),
