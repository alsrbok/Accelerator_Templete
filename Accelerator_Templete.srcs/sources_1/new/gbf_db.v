...
// Module: gbf_db
// Description:
//		Global Buffer Module for Activation and Weight and Psum
//		It contains two gbf_wrappers = Double buffering
//		
//
...
module gbf_db #(parameter DATA_BITWIDTH = 512,
    ...
    gbf_wrapper #(.DATA_BITWIDTH(DATA_BITWIDTH), .ADDR_BITWIDTH(ADDR_BITWIDTH), .DEPTH(DEPTH)) gbf_buffer1(.clk(clk), .ena(en1a),
    .enb(en1b), .wea(we1a), .addra(addr1a), .addrb(addr1b), .dia(w_data1a), .dob(r_data1b));

    gbf_wrapper #(.DATA_BITWIDTH(DATA_BITWIDTH), .ADDR_BITWIDTH(ADDR_BITWIDTH), .DEPTH(DEPTH)) gbf_buffer2(.clk(clk), .ena(en2a),
