...
module PE_new #(parameter IN_BITWIDTH = 16,         //For activation. weight, partial psum
          ...
          ( input clk,
            ...
            input wgt_en, wgt_sel, input [WGT_ADDR_BITWIDTH-1:0] wgt_addr1, wgt_addr2, input [IN_BITWIDTH-1:0] wgt_data1,wgt_data2,
            input psum_en, input [PSUM_ADDR_BITWIDTH-1:0] psum_addr1, psum_addr2, psum_write_addr,
    ...
    rf_iw_sync_dpdb #(.DATA_BITWIDTH(IN_BITWIDTH), .ADDR_BITWIDTH(ACTV_ADDR_BITWIDTH), .DEPTH(ACTV_DEPTH)
     ) rf_actv(.clk(clk), .reset(reset), .write_sel(actv_sel), .write_en(actv_en), .addr1(actv_addr1), .w_data1(actv_data1), 
    ...
    rf_iw_sync_dpdb #(.DATA_BITWIDTH(IN_BITWIDTH), .ADDR_BITWIDTH(WGT_ADDR_BITWIDTH), .DEPTH(WGT_DEPTH)
     ) rf_wgt(.clk(clk), .reset(reset), .write_sel(wgt_sel), .write_en(wgt_en), .addr1(wgt_addr1), .w_data1(wgt_data1), 
    ...
    rf_psum_sync_dpdb_new #(.DATA_BITWIDTH(OUT_BITWIDTH), .ADDR_BITWIDTH(PSUM_ADDR_BITWIDTH), .DEPTH(PSUM_DEPTH)
     ...
     .addr2(psum_addr2), .w_data2(psum2), .w_addr2(w_addr2), .r_data1(psum_in1), .r_data2(psum_in2), .out1(psum_out1), .out2(psum_out2));
    ...
    //MAC for PE
    MAC_new #(.IN_BITWIDTH(IN_BITWIDTH), .OUT_BITWIDTH(OUT_BITWIDTH)) mac(.a_in(actv), .w_in(wgt), 
