...
// Description:
//		SRAM Module
//		It contains two true_dp_rams = Double buffering
//		
//
...
module sram_db #(parameter DATA_BITWIDTH  = 512,
    ...
    true_dp_ram #(.DATA_BITWIDTH(DATA_BITWIDTH), .ADDR_BITWIDTH(ADDR_BITWIDTH), .DEPTH(DEPTH)) sram_buffer1(.clk(clk), .ena(en1a),
    .enb(en1b), .wea(we1a), .web(we1b), .addra(addr1a), .addrb(addr1b), .dia(w_data1a), .dib(w_data1b), .doa(r_data1a), .dob(r_data1b));

    true_dp_ram #(.DATA_BITWIDTH(DATA_BITWIDTH), .ADDR_BITWIDTH(ADDR_BITWIDTH), .DEPTH(DEPTH)) sram_buffer2(.clk(clk), .ena(en2a),
